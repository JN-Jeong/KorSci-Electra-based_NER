...
df_NER = pd.read_csv("uniq_keywords, codes, uniq_token_keywords_1112.csv") 
df_summary = pd.read_csv("2014-2019_summary_1112.csv")
...
save_filename = 'NER_ids_label_rev_index_1112.csv' # 1030은 키워드 길이 내림차순 정렬 안되어있었음, 1102는 키워드 길이 내림차순 정렬
                                                   # 1106는 1102+mecab+최신 vocab 적용, 1107은 1106 + 키워드 추가 정제
                                                   # 1107은 mecab 적용하는 과정에서 오류가 있었음
                                                   # 1112는 오류 수정 + 18, 19년 데이터 추가한 버전(키워드 정제는 안함)

keywords = df_NER['keyword']
codes = df_NER['code']
token_keywords = df_NER['token_keyword']
summarys = df_summary['summary']
# token_summarys = df_summary['token_summary']
token_summarys = df_summary['mecab_summary']
...
#     token_summary = tokenizer.tokenize(' '.join(mecab.morphs(summary)))
    token_summary = token_summary[1:-1].replace("'", "").replace(",,", "▁").replace(",", "").replace("▁", ",").split()
    y_data = []
    str_y_data = []
    ...
    with open("ner_to_index_rev_1112.json", 'rb') as f:
