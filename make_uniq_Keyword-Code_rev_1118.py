...
print("중복 제거 전 summarys 길이 : ", len(df_summary))
df_summary = df_summary.drop_duplicates(['summary']) # 요약 중복 제거
print("중복 제거 후 summarys 길이 : ", len(df_summary))
...
for i in range(len(df_key_code)):
    if not check_hangul(df_key_code['keyword'].iloc[i]):
        idx.append(i)
df_key_code = df_key_code.drop(df_key_code.index[idx])
...
for keyword, code in tqdm(zip(keywords, codes), total = len(codes)):
    ...
    for word in keyword:
        # 약간의 정제
        word = word.strip()
        if not check_hangul(word): # 키워드에 한글이나 영어가 포함되어 있지 않으면 제외
            continue
...
for i in tqdm(range(len(df_summary))):
    mecab_summary = mecab.morphs(df_summary['summary'].iloc[i]) # mecab morphs 적용
