...
df_NER = pd.read_csv("uniq_keywords, codes, uniq_token_keywords_1107.csv") 
df_summary = pd.read_csv("2014-2017_summary_1105.csv")
...
save_filename = 'NER_ids_label_rev_index_1107.csv' # 1030은 키워드 길이 내림차순 정렬 안되어있었음, 1102는 키워드 길이 내림차순 정렬
...
#     token_summary = tokenizer.tokenize(summary)
    token_summary = token_summary[1:-1].replace("'", "").replace(",", "").split()
    y_data = []
    str_y_data = []
